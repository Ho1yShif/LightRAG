...
interface AuthState {
  ...
  apiKeyRequired: boolean; // Server-driven API-key-only mode flag (from /auth-status; not persisted)
  ...
  setApiKeyRequired: (apiKeyRequired: boolean) => void; // Set API-key-only mode flag from server
}
